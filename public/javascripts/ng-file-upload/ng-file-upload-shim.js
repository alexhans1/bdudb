...
(function() {
  ...
  function patchXHR(fnName, newFn) {
    window.XMLHttpRequest.prototype[fnName] = newFn(
      window.XMLHttpRequest.prototype[fnName],
    );
  }
    ...
    patchXHR('getResponseHeader', function(orig) {
      return function(h) {
        return this.__fileApiXHR && this.__fileApiXHR.getResponseHeader
          ? this.__fileApiXHR.getResponseHeader(h)
          : orig == null
          ? null
          : orig.apply(this, [h]);
      };
    ...
    patchXHR('getAllResponseHeaders', function(orig) {
      return function() {
        return this.__fileApiXHR && this.__fileApiXHR.getAllResponseHeaders
          ? this.__fileApiXHR.getAllResponseHeaders()
          : orig == null
          ? null
          : orig.apply(this);
      };
            ...
            complete: function(err, fileApiXHR) {
              ...
              if (fileApiXHR.status !== undefined)
                redefineProp(xhr, 'status', function() {
                  return fileApiXHR.status === 0 && err && err !== 'abort'
                    ? 500
                    : fileApiXHR.status;
                });
              ...
              var resp =
                fileApiXHR.responseText ||
                (err && fileApiXHR.status === 0 && err !== 'abort'
                  ? err
                  : undefined);
...
(function() {
  ...
  function isInputTypeFile(elem) {
    return (
      ...
      elem.attr('type') &&
      elem.attr('type').toLowerCase() === 'file'
    );
  }
        ...
        if (elem.attr('disabled')) {
          ...
          if (!isInputTypeFile(elem)) {
            label
              ...
              .css('top', getOffset(elem[0]).top + 'px')
              .css('left', getOffset(elem[0]).left + 'px')
              .css('width', elem[0].offsetWidth + 'px')
              .css('height', elem[0].offsetHeight + 'px')
              .css('filter', 'alpha(opacity=0)')
              .css('display', elem.css('display'))
              .css('overflow', 'hidden')
              .css('z-index', '900000')
