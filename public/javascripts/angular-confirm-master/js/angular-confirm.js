    ...
    function($rootScope, $ngConfirmDefaults, $ngConfirmBase) {
      return function(options, options2, option3) {
        if (typeof options == 'string') {
          ...
          if (typeof options2 == 'string') options['title'] = options2 || false;
          else options['title'] = false;
          if (typeof options2 == 'object') options['scope'] = options2;
          if (typeof option3 == 'object') options['scope'] = option3;
        }
    ...
    function(
      ...
      ngConfirmBase.prototype = {
        ...
        _prepare: function() {
          ...
          $q.all([this._contentReady.promise, this._modalReady.promise]).then(
            function() {
              if (that.isAjax) {
          ...
          if (this.contentUrl) {
            ...
            $templateRequest(contentUrl).then(
              function(html) {
                that.content = html;
            ...
            if (typeof this.content == 'function') content = this.content();
        ...
        loading: function(show) {
          this.isLoading = show;
          if (show) this.$confirmBox.addClass(this._loadingClass);
          else this.$confirmBox.removeClass(this._loadingClass);
        },
        /**
         * Set content to DOM.
         * @param contentHtml
        ...
        setTypeAnimated: function(state) {
          if (state) this.$confirmBox.addClass(this._setTypeAnimatedClass);
          else this.$confirmBox.removeClass(this._setTypeAnimatedClass);
        },
        _pTitleClass: '',
        /**
        ...
        setBoxWidth: function(units) {
          if (this.useBootstrap) {
            $log.warn(
              'Cannot set boxWidth as useBootstrap is set to true. use columnClass instead.',
            );
            return;
          }
        ...
        setContainerFluid: function(state) {
          if (!this.useBootstrap) {
            $log.warn(
              'Cannot set containerFluid as useBootstrap is set to false.',
            );
            return;
          }
          ...
          if (state) {
            ...
            this.$bs3Container
              .removeClass(this.bootstrapClasses.containerFluid)
        ...
        setColumnClass: function(colClass) {
          if (!this.useBootstrap) {
            $log.warn(
              'Cannot set columnClass as useBootstrap is set to false, use bixWidth instead',
            );
            return;
          }
        ...
        setIcon: function(iconClass) {
          if (typeof iconClass == 'function') iconClass = iconClass();
        ...
        setCloseIcon: function(strFunc) {
          if (this._buttonCount == 0 && strFunc == null) strFunc = true;

          if (!!strFunc) this.$closeIcon.removeClass(this._hideClass);
          else this.$closeIcon.addClass(this._hideClass);
        ...
        setDismissAnimation: function(animation) {
          ...
          this.$confirmBox.removeClass(this._pSetDismissAnimation).addClass(c);
        ...
        _prefixThis: function(str, pr) {
          ...
          angular.forEach(str, function(a, k) {
            if (a.indexOf(pr) == -1) str[k] = pr + a.trim();
          });
        ...
        _setButtons: function(buttons) {
          ...
          angular.forEach(buttons, function(button, key) {
            ...
            if (typeof button.show == 'undefined') button.show = true;
            ...
            var button_el = angular.element(
              '<button type="button" class="btn"><span class="ng-confirm-btn-text"></span></button>',
            );
            ...
            buttons[key].setDisabled = function(state) {
              if (state) button_el.attr('disabled', 'disabled');
              else button_el.removeAttr('disabled');
              buttons[key].disabled = state;
            };
            buttons[key].setShow = function(state) {
              if (state) button_el.removeClass(self._hideClass);
              else button_el.addClass(self._hideClass);
        ...
        setRtl: function(state) {
          if (state) this.$el.addClass(this._rtlClass);
          else this.$el.removeClass(this._rtlClass);
        ...
        _unBindEvents: function() {
          ...
          if (this._digestWatchUnRegister) this._digestWatchUnRegister();
        },
        _reactOnKey: function(e) {
            ...
            if (this.escapeKey == true) {
              ...
              if (buttonName) {
                if (!angular.isDefined(this.buttons[buttonName])) {
                  $log.warn(
                    'Invalid escapeKey, no buttons found with name ' +
                      buttonName,
                  );
                } else {
          ...
          angular.forEach(this.buttons, function(button, key) {
            if (button.keys.indexOf(keyChar) != -1) that._buttonClick(key);
          });
        },
        _scrollPaneClick: function() {
          ...
          else str = this.backgroundDismiss;
        ...
        _closeClick: function() {
          ...
          if (buttonName) {
            ...
            shouldClose =
              typeof btnResponse == 'undefined' || !!btnResponse == true;
          }
          if (shouldClose) {
            this.close();
          }
        ...
        _buttonClick: function(buttonKey) {
          ...
          else this.scope.$apply(); // if there are changes in the scope. apply it
        ...
        setDialogCenter: function(where) {
          ...
          if (boxHeight > windowHeight - totalOffset || !this.alignMiddle) {
            ...
            angular
              .element('body')
              .addClass('ng-confirm-no-scroll-' + this._id);
          } else {
            style = {
            ...
            angular
              .element('body')
              .removeClass('ng-confirm-no-scroll-' + this._id);
          }
        ...
        _open: function() {
          ...
          setTimeout(function() {
            ...
            self.$confirmBox.css(
              self._getCSS(self.animationSpeed, self.animationBounce),
            );
        ...
        _startCountDown: function() {
          ...
          if (!angular.isDefined(this.buttons[this._autoCloseKey])) {
            $log.error(
              'Auto close button "' + self._autoCloseKey + '" not defined.',
            );
            return;
          }

          var timer_el = angular.element(
            '<span class="ng-confirm-timer"></span>',
          );
        ...
        close: function() {
          ...
          var closeTimer = this.animationSpeed * 0.4;

          setTimeout(function() {
            self.$el.remove();
            self.closed = true;
            if (!self._providedScope) self.scope.$destroy();
            ...
            angular
              .element('body')
              .removeClass('ng-confirm-no-scroll-' + self._id);
